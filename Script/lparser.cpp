...
{
  Proto *f = fs->f;
	for ( int k = 0; k < f->strings.size(); ++k )
	{
		if ( *f->strings[k] == *s )
			return k;
	}
	f->strings.push_back( s );
  return f->strings.size() - 1;
}
...
static void singlevar (LexState *ls, TString *n, expdesc *var) {
  ...
  if (level >= 1)  /* neither local (0) nor global (-1)? */
    luaX_syntaxerror(ls, "cannot access a variable in outer scope", n->c_str());
...
static void pushupvalue (LexState *ls, TString *n) {
  ...
  if (level == -1) {  /* global? */
    if (fs->prev == NULL)
      luaX_syntaxerror(ls, "cannot access upvalue in main", n->c_str());
    v.u.index = string_constant(fs->prev, n);
  }
  else if (level != 1)
    luaX_syntaxerror(ls,
         "upvalue must be global or local to immediately outer scope", n->c_str());
...
static void pushclosure (LexState *ls, FuncState *func) {
  ...
  for (i=0; i<func->nupvalues; i++)
	...
	f->protos.push_back( func->nF );
  luaK_code2(fs, OP_CLOSURE, f->protos.size()-1, func->nupvalues);
}


static void open_func (LexState *ls, FuncState *fs) {
	int nF = luaF_newproto(ls->L);
  Proto *f = ls->L->protos[ nF ];
...
static void close_func (LexState *ls) {
  ...
  f->lineinfo.push_back( MAX_INT );  /* end flag */
  f->bCreated = true;
  ls->fs = fs->prev;
...
int luaY_parser (lua_State *L, ZIO *z) {
