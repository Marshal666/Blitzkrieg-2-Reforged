...
static int number_constant (FuncState *fs, Number r) {
  ...
  Proto *f = fs->f;
  int c = f->numbers.size();
  int lim = c < LOOKBACKNUMS ? 0 : c-LOOKBACKNUMS;
  while (--c >= lim)
    if (f->numbers[c] == r) return c;
  /* not found; create a new entry */
  f->numbers.push_back( r );
  c = f->numbers.size() - 1;
  return c;
}
...
static void codelineinfo (FuncState *fs) {
  ...
  if (ls->lastline > fs->lastline) {
    if (ls->lastline > fs->lastline+1)
      f->lineinfo.push_back( -(ls->lastline - (fs->lastline+1)) );
    f->lineinfo.push_back( fs->pc );
...
int luaK_code2 (FuncState *fs, OpCode o, int arg1, int arg2) {
  ...
  if (optm) {  /* optimize: put instruction in place of last one */
			ASSERT( o != OP_CALL );
			ASSERT( (fs->f->code.back() & 63) != OP_CALL );
  ...
  codelineinfo(fs);
  /* push new instruction in code array */
	ASSERT( fs->pc == fs->f->code.size() );
	fs->f->code.push_back( i );
  return fs->pc++;
}
