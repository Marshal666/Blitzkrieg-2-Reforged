...
void luaX_syntaxerror (LexState *ls, const char *s, const char *token) {
  char buff[MAXSRC];
  luaO_chunkid(buff, ls->source->c_str(), sizeof(buff));
...
void luaX_error (LexState *ls, const char *s, int token) {
  ...
  if (buff[0] == '\0')
    luaX_syntaxerror(ls, s, &ls->L->Mbuffer[0] );
  else
...
#define checkbuffer(L, n, len)	if ( (len)+(n) > L->Mbuffer.size() ) \
...
static void read_number (LexState *LS, int comma, SemInfo *seminfo) {
  ...
  save(L, '\0', l);
  if (!luaO_str2d(&L->Mbuffer[0], &seminfo->r))
