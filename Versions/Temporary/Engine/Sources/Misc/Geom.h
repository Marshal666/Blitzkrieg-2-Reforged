...
//	Info
	...
	bool Touchs ( const CSpan  &span ) const
	{
		return minValue <= span.maxValue && maxValue >= span.minValue;
	}
	bool Intersects ( const CSpan  &span ) const
	{
		return minValue < span.maxValue && maxValue > span.minValue;
	}
...
//	Operators
	CSpan  &operator&=( const CSpan  &span )
	{
	...
	CSpan  operator&( const CSpan  &span ) const
	{
		CSpan  result = *this;
		result &= span;
		return result;
...
public:
	...
	{
		const float fDist = fabs( circle.center - center );
