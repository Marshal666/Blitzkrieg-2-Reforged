	...
	while 1 do
		...
		if (GetNUnitsInScriptGroup(6000,0)==1) then
			...
			if (TownCaptured == 0) then
				...
				LandReinforcementFromMap(0,"unserwaffe_tanks",1,4500);
				Cmd(ACT_SWARM,4500,0,GetScriptAreaParams("medium_tanks"));
				LandReinforcementFromMap(0,"unserwaffe_spg",1,4501);
				Cmd(ACT_SWARM,4501,0,GetScriptAreaParams("spg"));
				if (AI_on == 0) then
					StartThread(AI_obj1);
				end;
			else
...
function CompleteObjective1()
	while 1 do
		...
		if ((GetNUnitsInArea(0,"town",0)>0 and GetNUnitsInArea(1,"town",0)==0) and GetNUnitsInScriptGroup(6002,0)==1) then
			if (objective_0_Completed == 1) then
				...
				CompleteObjective(1);
				TownCaptured = 1;
				LandReinforcementFromMap(0,"townwaffe_art",3,7000);
				Cmd(ACT_MOVE,7000,0,GetScriptAreaParams("art"));
				Wait(2);
				Cmd(ACT_MOVE,7000,0,GetScriptAreaParams("art"));
				Wait(8);
				LandReinforcementFromMap(0,"townwaffe_spg",3,7002);
				Cmd(ACT_SWARM,7002,0,GetScriptAreaParams("town_spg"));
			end;
			Wait(4);
			break;
		end;
...
function CompleteObjective2()
	...
	while 1 do
		Wait(2);
		if ((GetNUnitsInScriptGroup(5006)==0) or (GetGameTime()-BeginTime > 240 and GetNScriptUnitsInArea(5006,"town",0)==0)) then
			Wait(2);
			CompleteObjective(2);
			Wait(3);
			Win(0);
...
function forceTo_key_railroad()
	while 1 do
		Wait(1);
		if (GetNUnitsInScriptGroup(6000,0)==1) then
			Trace("Tanks swarm to keybuilding on railroad...");
			Wait(20);
...
function capture_key_building_again()
	while 1 do
		...
		if (GetNUnitsInScriptGroup(6000,1)==1) then
			GiveReinforcementCalls(1,3);
			Trace("Enemy has got %g reinforcements",GetReinforcementCallsLeft(1));
			break;
		end;
	end;
end;
...
function wunderwaffe()
	while 1 do
		Wait(1);
		if (GetNUnitsInScriptGroup(1180,1)==0 and GetNUnitsInArea(0,"aa",0)>0 and GetNUnitsInArea(1,"aa",0)==0) then
			--if 1 then
			CompleteObjective(3);
			Trace("Stadt ist kaput!");
			StartThread(planes);
...
function planes()
	Trace("Thread planes has started...")
	while 1 do
		Wait(1);
