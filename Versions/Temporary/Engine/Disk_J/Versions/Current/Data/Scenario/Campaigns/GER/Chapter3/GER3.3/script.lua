function Start1()
	LandReinforcementFromMap( 0, "St1", 1, 300 ); 
	Wait( 2 );
	Cmd( 3, 300, 0, 1219, 1261 );
	Wait( 14 );
	LandReinforcementFromMap( 0, "St2", 1, 301 );
	Wait( 26 );	
	LandReinforcementFromMap( 0, "St3", 1, 302 );
	StartThread( Defead ); 
end;
-------------------------------------------------DEF_TM_180
function TM_180()
	while 1 do
		Wait( 2 );
		if (( GetNUnitsInScriptGroup( 501 ) < 1 ) and (GetIGlobalVar("temp.DOT", 1) ~= 2)) then
			Wait( 1 );
			SetIGlobalVar( "temp.TM180", 2 );
			FailObjective( 1 );
			Wait( 1 );
			Win( 1 );
			break;
		end;	
	end;
end;
-------------------------------------------------Obj0
function RevealObjective0()
	Wait( 10 );
	GiveObjective( 0 );
	StartThread( Objective0 );
end;

function Objective0()
	while 1 do
		Wait( 3 );
		if ((GetNUnitsInArea(0, "V1", 0) > 0) and (GetNUnitsInArea(1, "V1", 0) < 1) and (GetIGlobalVar("temp.TM180", 1) ~= 2)) then
			Wait( 2 );
			ChangePlayerForScriptGroup( 501, 0 );
			CompleteObjective( 0 );
			SetIGlobalVar( "temp.V1", 2 );
			Wait( 2 );
			GiveObjective( 1 );
			StartThread( Objective1 );
			StartThread( GER_SPG );           ----------------------------GERMAN_SPG
			break;
		end;	
	end;
end;
-------------------------------------------------Obj1
function Objective1()
	while 1 do
		Wait( 3 );
		if ((GetScriptObjectHPs(991) <= 0) and (GetScriptObjectHPs(992) <= 0))then
			CompleteObjective( 1 );
			SetIGlobalVar( "temp.DOT", 2 ); 
			Wait( 2 );
			GiveObjective( 2 );
			StartThread( Objective2 );
			break;
		end;	
	end;
end;
------------------------------------------------Obj2
function Objective2()
	while 1 do
		Wait( 3 );
		if (GetNUnitsInArea(1, "S1", 0) < 1) then
			CompleteObjective( 2 );
			Wait( 2 );
			Win( 0 );
			break;
		end;	
	end;
end;
------------------------------------------------------------------PART2
----------------------------------------RUS_TM_ATTACK
function R_Attack_Rand() 
	while 1 do
		Wait( 2 );
			if ((GetNUnitsInArea(0, "V1", 0) > 0) and (GetNUnitsInArea(1, "V1", 0) < 2) and (GetIGlobalVar("temp.DOT", 1) ~= 2)) then
			Wait( 1 );
			StartThread( Rand );
			break;
		end;
	end;
end;

function Rand()
local x = RandomInt(2);
	if x == 0 then
	StartThread( A1 );
	end;
	if x == 1 then
	StartThread( A2 );
	end;
end;

function A1()
	LandReinforcementFromMap( 1, "R_INF", 0, 701 );         
	Wait( 2 );
	ChangeFormation( 701, 3 );
	Wait( 3 );
	Cmd( 3, 701, 0, 2408, 1260 );
	QCmd( 3, 701, 0, 6633, 968 );
	Wait( 70 + RandomInt( 30 ) );
	StartThread( R_Attack_Rand );
end;

function A2()
	LandReinforcementFromMap( 1, "R_TANKS", 0, 702 );  
	Wait( 2 );
	Cmd( 3, 702, 0, 2408, 1260 );
	QCmd( 3, 702, 0, 6633, 968 );
	Wait( 70 + RandomInt( 30 ) );
	StartThread( R_Attack_Rand );
end;
-------------------------------------------SPG
function GER_SPG()
	LandReinforcementFromMap( 2, "G_SPG", 0, 121 ); 
	Cmd( 3, 121, 0, 3045, 1580 );
	QCmd( 8, 121, 0, 2450, 4288 );
	Wait( 5 );
	LandReinforcementFromMap( 2, "G_SPG", 0, 122 ); 
	Cmd( 3, 122, 0, 3785, 2414 );
	QCmd( 8, 122, 0, 3072, 3019 );
	StartThread( GER_SPG_2 );
end;

function GER_SPG_2()
	while 1 do
		Wait( 2 );
		if (( GetNUnitsInScriptGroup( 121 ) < 1 ) and ( GetNUnitsInScriptGroup( 122 ) < 1 )) then
			Wait( 20 );
			StartThread( GER_SPG );
			break;
		end;	
	end;
end;
-----------------------------------------GERMAN_TANK_ATTACK_1

function GERMAN_ATTACK_S() 
	while 1 do
		Wait( 2 );
			if ((GetIGlobalVar("temp.DOT", 1) == 2) and ( GetNUnitsInScriptGroup( 124 ) <= 1 )) then
			Wait( 2 );
			StartThread( Rand_G );
			break;
		end;
	end;
end;

function Rand_G()
local x = RandomInt(2);
	if x == 0 then
	StartThread( Z1a );
	end;
	if x == 1 then
	StartThread( Z2b );
	end;
end;
-----------------------------------Road1
function Z1a()
	LandReinforcementFromMap( 2, "G_TANKS", 2, 124 ); 
	LandReinforcementFromMap( 2, "G_AVIA", 2, 125 );        
	Wait( 3 );
	Cmd( 3, 125, 0, GetScriptAreaParams( "AVIA" ) );
	Cmd( 3, 124, 0, GetScriptAreaParams( "R1_1" ) );
	StartThread( Z1_1 );
	StartThread( GERMAN_ATTACK_S );
end;

function Z1_1() 
	while 1 do
		Wait( 2 );
			if (GetNScriptUnitsInArea(124, "R1_1") > 0) then
			Wait( 15 );
			Cmd( 3, 124, 0, GetScriptAreaParams( "R1_2" ) );
			StartThread( Z1_2 );
			break;
		end;
	end;
end;

function Z1_2() 
	while 1 do
		Wait( 2 );
			if (GetNScriptUnitsInArea(124, "R1_2") >0) then
			Wait( 15 );
			Cmd( 3, 124, 0, GetScriptAreaParams( "R1_3" ) );
			break;
		end;
	end;
end;
-------------------------------------Road2
function Z2b()
	LandReinforcementFromMap( 2, "G_TANKS", 2, 124 );
	LandReinforcementFromMap( 2, "G_AVIA", 2, 125 );
	Wait( 3 );
	Cmd( 3, 125, 0, GetScriptAreaParams( "AVIA" ) );
	Cmd( 3, 124, 0, GetScriptAreaParams( "R2_1" ) );
	StartThread( Z2_1 );
	StartThread( GERMAN_ATTACK_S );
end;

function Z2_1() 
	while 1 do
		Wait( 2 );
			if (GetNScriptUnitsInArea(124, "R2_1") >0) then
			Wait( 15 );
			Cmd( 3, 124, 0, GetScriptAreaParams( "R2_2" ) );
			StartThread( Z2_2 );
			break;
		end;
	end;
end;

function Z2_2() 
	while 1 do
		Wait( 2 );
			if (GetNScriptUnitsInArea(124, "R1_2") >0) then
			Wait( 15 );
			Cmd( 3, 124, 0, GetScriptAreaParams( "R2_3" ) );
			QCmd( 3, 124, 0, GetScriptAreaParams( "R2_4" ) );
			break;
		end;
	end;
end;
------------------------------------------GERMAN_INFAN_ATTACK
function GERMAN_INFAN() 
	while 1 do
		Wait( 2 );
			if (GetNUnitsInArea(2, "A1", 0) > 0) then
			Wait( 20 );
			StartThread( GERMAN_INFANGO_2 );
			break;
		end;
	end;
end;

function GERMAN_INFANGO_2()
	LandReinforcementFromMap( 2, "G_INF", 1, 124 ); 
	Wait( 2 );
	ChangeFormation( 124, 3 );
	Wait( 3 );
	Cmd( 3, 124, 100, 6557, 6771 );
end;
------------------------------------------Winners
function Defead()
    while 1 do
        if (( GetNUnitsInParty(0) < 1) and ( GetReinforcementCallsLeft( 0 ) == 0 )) then
		Wait(3);
		Win(1);
		return 1;
		end;
	Wait(5);
	end;
end;
-------------------------------------------Armor_Train
function A_TR()
	while 1 do
		Wait( 2 );
		if ( GetNUnitsInScriptGroup( 621 ) > 0 ) then
			StartThread( TRAIN );
			break;
		end;	
	end;
end;

function TRAIN()
    Wait( 180 + RandomInt( 120 ) );
	Cmd( 0, 621, 0, GetScriptAreaParams( "AT" ) );
end;
-------------------------------------------------
StartThread( Start1 );
StartThread( RevealObjective0 );
StartThread( TM_180 );
StartThread( R_Attack_Rand );
StartThread( GERMAN_ATTACK_S );
StartThread( GERMAN_INFAN );
StartThread( A_TR );



