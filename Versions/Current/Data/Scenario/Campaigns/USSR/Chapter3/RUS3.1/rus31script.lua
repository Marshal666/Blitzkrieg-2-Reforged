...
function CallGermanReinforcement ()
	...
	local level = GetDifficultyLevel();
	...
	for i = 1, 4 + level do
		...
		GiveReinforcementCalls (ID_GERMANS, 3);
...
function CheckObjective1 ()
	while 1 do
		Wait (5);
		if GetNUnitsInArea (ID_GERMANS, "SouthernVillage", 0)==0 and 
			GetNUnitsInScriptGroup(ID_S_KEY, ID_PLAYER) > 0 then
			OBJ1_COMPLETED = 1;
...
function CheckObjective2 ()
	while 1 do
		Wait (5);
		if GetNUnitsInArea (ID_GERMANS, "NorthernVillage", 0)==0 and 
			GetNUnitsInScriptGroup(ID_N_KEY, ID_PLAYER) > 0 then
			OBJ2_COMPLETED = 1;
...
function BridgeMission ()
	...
	DamageScriptObject( ID_BRIDGE_SOUTH, 2*GetScriptObjectHPs(ID_BRIDGE_SOUTH));
	DamageScriptObject( ID_BRIDGE_NORTH, 2*GetScriptObjectHPs(ID_BRIDGE_NORTH));
	
	ArmorTrainCompeted = 0;

	Wait (30);

	while 1 do
		Wait (2);
		if GetScriptObjectHPs(ID_BRIDGE_SOUTH) > 0 or GetScriptObjectHPs (ID_BRIDGE_NORTH) > 0 then
			break;
		end;
	end;
	...
	while 1 do
		Wait (2);
		if GetNUnitsInArea (ID_PLAYER, "Train", 0) > 0 and ArmorTrainCompeted == 0 then
			ArmorTrainCompeted = 1;
			ChangePlayerForScriptGroup (ID_MEGATRAIN, ID_PLAYER);
			GiveObjective (ID_OBJ_ARMOR_TRAIN);
			CompleteObjective (ID_OBJ_ARMOR_TRAIN);
			return 1;
		end;
	end;
