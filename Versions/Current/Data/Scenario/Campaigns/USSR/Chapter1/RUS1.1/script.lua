function Init()
	...
	Objectives.Secret = OBJ_SECRET_START;
	...
	-- pz_ii_ausf_g_3 - light
	-- pz_vi_tiger_ausf_a_h_1 - medium
	-- pz_vi_tiger_ausf_a_h_2 - heavy
	
	AddWestEnemyWave(130, "main_squad");
	AddWestEnemyWave(0, "main_squad");
	
	AddWestEnemyWave(45, "main_squad");
	AddWestEnemyWave(20, "tanks_light", "WEST_02_01");
	
	AddWestEnemyWave(45, "tanks_light", "WEST_01_01");
	AddWestEnemyWave(0, "tanks_light", "WEST_02_01");
	
	AddWestEnemyWave(45, "main_squad");
	AddWestEnemyWave(20, "tanks_light", "WEST_01_01");
	AddWestEnemyWave(0, "tanks_light", "WEST_02_01");
	
	AddWestEnemyWave(45, "main_squad");
	AddWestEnemyWave(20, "tanks_medium", "WEST_02_01");

	AddWestEnemyWave(45, "tanks_medium", "WEST_01_01");
	AddWestEnemyWave(0, "tanks_medium", "WEST_02_01");
	
	local nLevel = GetDifficultyLevel();
	
	if (nLevel == DLEVEL_NORMAL) or (nLevel == DLEVEL_HARD) then
		AddWestEnemyWave(45, "main_squad");
		AddWestEnemyWave(20, "tanks_heavy", "WEST_02_01");
	end;
	if (nLevel == DLEVEL_HARD) then
		AddWestEnemyWave(45, "main_squad");
		AddWestEnemyWave(20, "tanks_heavy", "WEST_01_01");
		AddWestEnemyWave(0, "tanks_heavy", "WEST_02_01");
	end;
...
function ProcessEastAlly()
	while 1 do
		...
		if (Objectives.State == OBJ_STATE_2) and (EastAlly.State == EAST_ALLY_STATE_NONE) then
			...
			LandReinforcementFromMap(2, "tanks_medium", 1, EastAlly[1].ScriptID);
			local x, y, h;
...
function ProcessSecretObjective()
	while 1 do
		Wait(1);
		if Objectives.Secret == OBJ_SECRET_START then
			if IsSomeBodyAlive( 1, SECRET_GOAL_SCRIPT_ID ) ~= 1 then
				Objectives.Secret = OBJ_SECRET_DONE;
