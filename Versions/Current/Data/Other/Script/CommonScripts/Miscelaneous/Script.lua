...
function IsObjectInCircle( object, x, y, radius )
...
function Patrol( unit, iDispersion, ... )
	...
	UnitCmd( ACT_PATROL, unit, arg[1] + RandomInt( 2 * iDispersion ) - iDispersion , arg[2] + RandomInt( 2 * iDispersion ) - iDispersion );
	if ( arg.n > 2 ) then
		for i = 1, arg.n / 2 do
			UnitQCmd( ACT_PATROL, unit, arg[(i * 2) + 1] + RandomInt( 2 * iDispersion ) - iDispersion , arg[(i + 1) * 2] + RandomInt( 2 * iDispersion ) - iDispersion );
		end;
	end;
end;
...
function GetNUnitsInArea1( player, scriptarea )
local units = {};
local rpgstats = {};
local cnt = 0;
	...
	while ( stop == 0 ) do
		...
		for i = 1, array.n do
			if ( IsAlive( array[i] ) == 1 ) then
				if ( GetUnitState( array[i] ) == state ) then
					stop = 0;
					break;
				end;
			end;
