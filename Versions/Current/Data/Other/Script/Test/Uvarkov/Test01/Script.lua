SetIGlobalVar( "test", 16 );

Cmd( 0, 0, 1200, 400 ); -- command/id

--Cmd( 16, 2, 1200, 400 ); -- гаубица

Wait( 5 );

ObjectiveChanged( 1, 2 );
--Wait( 5 );
ObjectiveChanged( 2, 3 );
--Wait( 5 );
ObjectiveChanged( 0, 1 );
--Wait( 5 );
ObjectiveChanged( 3, 1 );

Wait( 5 );

ObjectiveChanged( 3, 0 );

testVar = GetIGlobalVar( "test", 0 );

Cmd( testVar, 2, 1200, 400 ); -- гаубица

--ObjectiveChanged( 3, 2 );

--ObjectiveChanged( 4, 2 );

--ObjectiveChanged( 5, 2 );

--ObjectiveChanged( 6, 2 );


--ObjectiveChanged( 7, 2 );

--ObjectiveChanged( 8, 1 );
