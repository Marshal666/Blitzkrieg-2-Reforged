...
function Player() 
	while 1 do
		Wait( 2 );
        if ((GetNUnitsInArea(0, "WH",0) < 1) and (GetNUnitsInArea(2, "WH",0) < 1) and (GetNUnitsInArea(1) > 0)) then
			Wait( 1 );
			Win(1);
			break;
		end;
...
function Attack_2()
	Wait( 5 );
	LandReinforcementFromMap (1, 1, 2, 2000 ); 
	Wait( 3 );
...
function Attack_3()
	Wait( 135 );
	LandReinforcementFromMap (1, 0, 1, 1100 ); 
	Wait( 3 );
...
function Attack_4()
	Wait( 105 );
	LandReinforcementFromMap (1, 1, 0, 2100 ); 
	Wait( 3 );
...
function Attack_5()
	Wait( 235 );
	LandReinforcementFromMap (1, 0, 0, 1200 ); 
	Wait( 3 );
...
function Attack_6()
	Wait( 155 );
	LandReinforcementFromMap (1, 1, 2, 2200 ); 
	Wait( 3 );
...
function Attack_7()
	Wait( 245 );
	LandReinforcementFromMap (1, 2, 1, 1700 ); 
	Wait( 2 );
...
function Air_Attack()
	...
	LandReinforcementFromMap (1, 3, 1, 3000 ); 
	Wait( 2 );
