...
{
	NI_ASSERT( false, "Can convert only to CVec2 and CVec3!" );
}
template <>
...
//
template <class TPoint>
inline const TPoint GetPointType( const CVec3 &vec, TPoint *pTargetType )
{
	NI_ASSERT( false, "Can convert only to CVec2 and CVec3!" );
}
template <>
...
EXTERNVAR const EClassifyPolygon NEGATIVE_CLASSIFY_POLYGON[CP_COUNT + 1];
													 ...
													 Type &rApplyFunctional )						//функционал
{
	...
	for ( int nXIndex = indices.minx; nXIndex < indices.maxx; ++nXIndex )
	{
		for ( int nYIndex = indices.miny; nYIndex < indices.maxy; ++nYIndex )
		{
			...
			TPoint point = GetPointType( vPoint, static_cast<TPoint*>( 0 ) );
													...
													Type &rApplyFunctional )						//функционал
{
	...
	for ( int nXIndex = indices.minx; nXIndex < indices.maxx; ++nXIndex )
	{
		for ( int nYIndex = indices.miny; nYIndex < indices.maxy; ++nYIndex )
		{
			...
			TPoint tileCenter = GetPointType( vTileCenter, static_cast<TPoint*>( 0 ) );
